...
$staging = Join-Path ([System.IO.Path]::GetTempPath()) "severaludo_clock_sync_210"
...
if (-not (Test-Path -LiteralPath $source -PathType Leaf)) { throw "Clock Sync 2.1.0 source was not found." }
...
if ($LASTEXITCODE -ne 0) { throw "Clock Sync 2.1.0 compilation failed." }
...
if ($LASTEXITCODE -ne 0) { throw "Clock Sync 2.1.0 wrapper validation failed." }
