...
if (-not (Test-Path -LiteralPath $source -PathType Leaf)) { throw "Clock Sync 2.2.9 source was not found." }
...
if ($LASTEXITCODE -ne 0) { throw "Clock Sync 2.2.9 compilation failed." }
...
if ($LASTEXITCODE -ne 0) { throw "Clock Sync 2.2.9 wrapper validation failed." }
