...
class CoreSmokeTests(unittest.TestCase):
    ...
    def test_clock_receiver_and_sync_protocol(self):
        ...
        with TestClient(app) as client:
            ...
            self.assertEqual(ping.json()["clock_sync_version"], "2.2.9")
    ...
    def test_approved_event_catalog_repairs_a_missing_row_without_duplicates(self):
        with TestClient(app):
            with SessionLocal() as session:
                ...
                long_title=max(events,key=lambda item:len(str((item.data or {}).get("source_event_name") or "")))
                self.assertGreater(len(long_title.data["source_event_name"]),240)
                self.assertLessEqual(len(long_title.label),240)
                self.assertTrue(long_title.label.endswith("…"))
