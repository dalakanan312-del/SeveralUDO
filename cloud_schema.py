...
TABLES=["settings","sims","households","pregnancies","rolls","relationships","events","event_results","rules","calendar_rows","raw_import_rows","sim_photos","roll_rule_eras","roll_rule_values"]

def create_registry(raw_conn):
    with raw_conn.cursor() as cur:
        cur.execute("""CREATE TABLE IF NOT EXISTS public.decades_saves(save_id TEXT PRIMARY KEY,name TEXT NOT NULL,schema_name TEXT NOT NULL UNIQUE,created_at TIMESTAMPTZ NOT NULL DEFAULT now(),updated_at TIMESTAMPTZ NOT NULL DEFAULT now(),source_note TEXT)""")
    raw_conn.commit()

def create_save_schema(raw_conn, schema_name):
    from psycopg import sql
    with raw_conn.cursor() as cur:
        cur.execute(sql.SQL("CREATE SCHEMA IF NOT EXISTS {}").format(sql.Identifier(schema_name)))
        cur.execute(sql.SQL("SET search_path TO {}, public").format(sql.Identifier(schema_name)))
        for ddl in TABLE_DDLS: cur.execute(ddl)
    raw_conn.commit()
