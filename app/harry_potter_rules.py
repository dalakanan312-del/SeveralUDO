...
_ROWS = (
    ...
    ("HP-04","Blood Status","Magical identity","Recommended","","Birth or ancestry discovery","Pureblood requires all four grandparent positions to be confirmed spellcasters. Half-Blood requires magical ancestry or ability and a confirmed non-spellcaster grandparent. A magical child of two recorded Muggles is Muggle-Born. Missing ancestry remains Unknown; two magical parents alone do not establish Pureblood. Squibs retain ancestry but do not count as spellcaster grandparents."),
...
def sync_pack(session: Session, save: ChronicleSave, selected: list[str]) -> int:
    ...
    for rule in ALL_RULES:
        ...
        if rule["code"] == "HP-04" and data.get("rule_text") == "Assign Pureblood from fully magical recent ancestry, Half-Blood from mixed magical/Muggle ancestry, Muggle-Born to a magical child of two Muggles, and Muggle to a non-magical child. Squibs retain magical ancestry.":
            desired.update(rule_text=rule["rule_text"], result_rules=rule["rule_text"])
