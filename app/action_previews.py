...
def describe(plan,kind):
    ...
    for change in plan['records']:
        ...
        if row['kind']=='roll':
            ...
            for key,label in [('hp_blood_status','Blood status (rolled ancestry)' if d.get('hp_blood_status_mode')=='rolled' else 'Blood status'),('hp_magical_ability','Magical ability'),('hp_hogwarts_house','Hogwarts house'),('species_occult','Occult status'),('occult_alignment','Occult alignment'),('pregnancy_allowance_count','Pregnancy allowance'),('fertility_status','Fertility status')]:
