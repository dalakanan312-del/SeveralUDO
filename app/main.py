...
templates.env.globals.update(
    ...
    preference_groups=trait_visibility.preference_groups,
...
async def edit_sim(request: Request, sim_id: str):
    ...
    with db() as session:
        ...
        if form.get("trait_sections") == "1":
            reviewed_traits = trait_visibility.reviewed(form, record.data.get("game_trait_details"), prefix="game_", values=record.data.get("game_traits"))
...
async def accept_automation(request: Request, candidate_id: str):
    ...
    with db() as session:
        ...
        if action=="save_portrait":
            ...
            if form.get("trait_sections") == "1":
                reviewed_traits = trait_visibility.reviewed(form, payload.get("trait_details"), values=payload.get("traits"))
...
def download_clock_sync_component(request: Request, component: str, game_mode: str = game_modes.SIMS4):
    ...
    media_type = "application/zip" if component == "script" and mode == game_modes.SIMS4 else "application/octet-stream" if component == "script" else "text/plain; charset=utf-8"
