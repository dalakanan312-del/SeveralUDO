...
const AD_PREFERENCE_KEY='decades-ad-preference';
function advertisingConfig(){const source=document.querySelector('#decades-ad-config');if(!source)return null;try{return JSON.parse(source.textContent||'{}');}catch(_error){return null;}}
function advertisingPreference(){try{return localStorage.getItem(AD_PREFERENCE_KEY)||'';}catch(_error){return '';}}
function storeAdvertisingPreference(value){try{localStorage.setItem(AD_PREFERENCE_KEY,value);}catch(_error){}}
function showAdvertisingDialog(){const dialog=document.querySelector('#ad-consent-dialog');if(dialog)dialog.hidden=false;}
function hideAdvertisingDialog(){const dialog=document.querySelector('#ad-consent-dialog');if(dialog)dialog.hidden=true;}
function loadOptInAdvertising(){const config=advertisingConfig();if(!config?.available||document.querySelector('script[data-decades-ad-script]'))return;const script=document.createElement('script');script.async=true;script.crossOrigin='anonymous';script.dataset.decadesAdScript='true';script.src=`https://pagead2.googlesyndication.com/pagead/js/adsbygoogle.js?client=${encodeURIComponent(config.client)}`;script.addEventListener('load',()=>{document.querySelectorAll('[data-ad-zone]').forEach((zone)=>{zone.hidden=false;});document.querySelectorAll('.adsbygoogle:not([data-ad-initialized])').forEach((unit)=>{try{(window.adsbygoogle=window.adsbygoogle||[]).push({});unit.dataset.adInitialized='true';}catch(_error){}});},{once:true});document.head.appendChild(script);}
function initializeAdvertising(){const config=advertisingConfig();if(!config?.consent_available)return;const preference=advertisingPreference();if(preference==='accepted')loadOptInAdvertising();else if(!preference)showAdvertisingDialog();document.querySelectorAll('[data-ad-preference]').forEach((control)=>{if(control.dataset.adReady==='true')return;control.dataset.adReady='true';control.addEventListener('click',(event)=>{event.preventDefault();const choice=control.dataset.adPreference;storeAdvertisingPreference(choice);hideAdvertisingDialog();if(choice==='accepted')loadOptInAdvertising();});});document.querySelectorAll('[data-manage-ad-preference]').forEach((control)=>{if(control.dataset.adReady==='true')return;control.dataset.adReady='true';control.addEventListener('click',showAdvertisingDialog);});}
initializeAdvertising();

document.addEventListener('htmx:afterSwap',initializeAdvertising);
...
const NAVIGATION_STATE_KEY='decades-navigation-groups:'+ (document.querySelector('#ui-context')?.dataset.user || 'signed-out');
...
function filterNavigation(value){const query=String(value||'').trim().toLowerCase();let matches=0;const saved=savedNavigationGroups();document.querySelectorAll('.nav-group').forEach((group)=>{let groupMatches=0;group.querySelectorAll('[data-nav-search]').forEach((link)=>{const match=!query||link.dataset.navSearch.toLowerCase().includes(query);link.hidden=!match;if(match&&query){groupMatches+=1;matches+=1;}});group.querySelectorAll('[data-nav-collection]').forEach(collection=>{collection.hidden=Boolean(query&&![...collection.querySelectorAll('a')].some(a=>!a.hidden));if(query&&!collection.hidden)collection.open=true;});group.hidden=Boolean(query&&!groupMatches);if(query&&groupMatches)group.open=true;else if(!query){group.open=Boolean(group.querySelector('.active')||(group.dataset.navGroup in saved?saved[group.dataset.navGroup]:group.open));}});const empty=document.querySelector('.nav-no-results');if(empty)empty.hidden=!query||matches>0;}
...
let trackerSearchTimer=0;
let trackerSearchController=null;
function initializeGlobalSearch(){
  const input=document.querySelector('#tracker-global-search');
  const results=document.querySelector('#tracker-global-results');
  ...
  const show=(items)=>{
    ...
    if(!items.length){const empty=document.createElement('p');empty.textContent='No matching records yet.';results.append(empty);}
    items.forEach((item)=>{const link=document.createElement('a');link.href=item.href;const title=document.createElement('strong');title.textContent=item.label;const detail=document.createElement('small');detail.textContent=`${item.kind} · ${item.detail}`;link.append(title,detail);results.append(link);});
    results.hidden=false;input.setAttribute('aria-expanded','true');
  };
  input.addEventListener('input',()=>{
    clearTimeout(trackerSearchTimer);const query=input.value.trim();
    if(query.length<2){hide();return;}
    trackerSearchTimer=setTimeout(async()=>{
      if(trackerSearchController)trackerSearchController.abort();trackerSearchController=new AbortController();
      try{const response=await fetch(`/api/search?q=${encodeURIComponent(query)}`,{signal:trackerSearchController.signal,headers:{Accept:'application/json'}});if(!response.ok)return;const data=await response.json();if(input.value.trim()===query)show(data.results||[]);}catch(_error){}
    },160);
  });
  input.addEventListener('keydown',(event)=>{if(event.key==='Escape'){hide();input.blur();}});
  document.addEventListener('click',(event)=>{if(!event.target.closest('.tracker-global-search'))hide();});
}
function rememberLastTrackerPage(){
  const saveId=document.body.dataset.saveId;if(!saveId||!location.pathname.startsWith('/p/'))return;
  try{localStorage.setItem(`decades-last-page:${saveId}`,location.pathname+location.search);}catch(_error){}
}
function restoreLastTrackerPage(){
  const link=document.querySelector('#resume-last-page'),saveId=document.body.dataset.saveId;if(!link||!saveId)return;
  try{const href=localStorage.getItem(`decades-last-page:${saveId}`);if(href&&href.startsWith('/p/')){link.href=href;link.hidden=false;}}catch(_error){}
}
document.addEventListener('keydown',(event)=>{if((event.ctrlKey||event.metaKey)&&event.key.toLowerCase()==='k'){const input=document.querySelector('#tracker-global-search');if(input){event.preventDefault();input.focus();input.select();}}});
initializeGlobalSearch();rememberLastTrackerPage();restoreLastTrackerPage();
document.addEventListener('htmx:afterSwap',()=>{initializeGlobalSearch();rememberLastTrackerPage();restoreLastTrackerPage();});
